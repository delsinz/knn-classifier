...
('young' or 'old') or three ('very young', 'old' and 'middle-age') categories. The classification method meant to be used was K-Nearest Neighbours. We could choose our similarity metrics, voting methods and evaluation methods for the dataset and validation framework for the dataset and we went with the following options:  \\


\begin{tabular}{|p{3cm}|p{3.5cm}|p{3.5cm}|p{3.5cm}|}
 \hline
 Similarity Metric & Euclidean Distance & Manhattan Distance & Minkowski Distance (with p = 0.5)\\
 \hline
 Voting Method & Equality Weight Majority & Inverse Distance & Inverse Linear Distance \\ 
 \hline
 Evaluation Metric & Accuracy and Error & Precision & Recall \\ 
 \hline
\end{tabular} 
\\\\ \\
...
\end{equation}
\\
(We take pow = 0.5 for Minkowski distance, as it lets us makes for a nice comparison with Euclidean since Euclidean is basically Minkowski with p = 2, {\bf when we refer to Minkowski in this report we will talk about the specific case with p = 0.5}) \\
...
An issue with distance metrics sometimes is the need to normalize the data before classification due to the possibility of one attribute dominating over others, but in our dataset the values are all constricted to a small enough space for it to not make much of a difference. 

\subsection{Euclidean - as the best}

Our choices of Euclidean, Minkowski (p = 0.5) and Manhattan Distance all of which try the distances in values of attributes for instances would give similar results with this dataset due to the values of distances being in a relatively small range but we conducted some experiments to determine which one is the best. The experiments (results in Table 1) show us that that the performance of all of the similarity metrics is very closely lumped together (as expected), but Eulidean seems to be slightly more accurate and have higher percision/recall so we think it is the best.

(Note: All measurements in the tables are averages of 10 runs with 10 fold cross validation being used for each run)
...
We also tested 10-Fold Cross Validation against holdout (split at 90:10), 5-Fold Cross Validation and 20-Fold Cross Validation and here is a graph of their accuracies for 10 different runs with abalone-2 and abalone-3 each (here k=31, similarity=euclidean and voting=inverse distance):
...
\caption{Comparison of accuracy of prediction over multiple runs for different validation methods with Abalone-3}
\end{figure}
...
\begin{tabular}{|p{2cm}|p{4cm}||p{2cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|}
 ...
 \hline
 Yes & k=29,voting=inverse distance &   euclidean & 0.7542 & 0.6234 & 0.6273\\
 \hline
 ...
 \hline
 No & k=29,voting=inverse distance &   minkowski & 0.7616 & 0.6251 & 0.6289\\
 \hline
 Yes & k=29,voting=inverse distance &   minkowski & 0.7578  & 0.6121 & 0.6219\\ 
 \hline
 No & k=29,voting=majority vote &   euclidean & 0.7631  & 0.6243 & 0.6321\\
 \hline
 Yes & k=29,voting=majority vote &   euclidean & 0.7564 & 0.6127 & 0.5987\\
 \hline
...
We see that for k around 29 we get a peak, and we chose that as our value of k, the accuracy drops off towards k=100 since we probably start getting a bias towards the majority class. 


\section{Appendix - A}

\subsection{Our voting methods}

We implemented three voting methods, one that just counts the majority class amongst the neighbours of the instance we are classifying i.e. equal weight, one that takes the inverse of the distance as the score and one that takes the inverse linear distance as the score. The formulas for the last two are:
\\\\
Inverse Distance: 
\begin{equation}
w_j = 1/(d_j + 0.5)  
...
Our experiments show that all voting methods work equally well with equal weight surprisingly getting the ever so slightly higher results. 
...
\begin{tabular}{|p{2cm}|p{4.5cm}||p{2cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|}
 ...
 \hline
 Abalone-3 & k=29,distance = euclidean & inverse distance & 0.7636 & .6417 & 0.6425\\
 \hline
...
We implement functions to calculate Accuracy, Precision and Recall (we macro-average the Precision and Recall, so as to not give too high a weight to the majority class). We also implement a function to find the error using the accuracy. In order to judge our performance we used the 1-R majority class voting as the baseline which has an accuracy of 0.65 and we comfortably beat it with an accuracy of 0.78 on average for Abalone-2. For Abalone-3, the 1-R method gives us a baseline accuracy of around 0.35 and our program clears that without too many issues. But for Abalone-3 our program struggles to find "middle-age" and "old" abalones as compared to "very-young" ones, probably because there isn't much difference in the values of the attributes for "old" and "middle-age" abalones. The fact that division in Abalone-2 was done such that we have a baseline of 65\% with the simple 1-R method tells us that this was the score to beat, and thus probably answers the challenge question. 
